...
import sipua.SipUAConnectionListener;
import sipua.SipUADeviceListener;
...
public class DeviceImpl implements IDevice, Serializable {
    ...
    public void initialize(Context context, SipProfile sipProfile) {
        ...
        sipManager = new SipManager(sipProfile, context);
    ...
    public void onSipMessage(final SipEvent sipEventObject) {
        ...
        if (sipEventObject.type == SipEventType.MESSAGE) {
            if (this.deviceListener != null)
                this.deviceListener.onSipUAMessageArrived(new SipEvent(this, SipEventType.MESSAGE, sipEventObject.content, sipEventObject.from));
    ...
    public void Call(String to) {
        try {
            ...
            sipManager.Call(to, this.soundManager.setupAudioStream(sipProfile.getLocalIp()));
        } catch (NotInitializedException | NullPointerException e) {
            e.printStackTrace();
        }
    }
